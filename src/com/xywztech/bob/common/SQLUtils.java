 ...
 * <pre>
 * Title:SQL工具类
 * Description: SQL相关的操作方法。
 * 
 * </pre>
 * 
 * @author mengzx mengzx@xywztech.com
 * @version 1.00.00
 * 
 *          <pre>
 * 修改记录
...
//				// 查询字段
//				queryField = "distinct "
//						+ sql.substring(findStrPosition(sql, "distinct") + 8,
//								findStrPosition(sql, "from")).trim();
//
//			} else {

				queryField = sql.substring(findStrPosition(sql, "select") + 6,
						findStrPosition(sql, "from")).trim();
			//}
			countBuff.append("select count(" + queryField + ") ");
	...
	public static boolean containsDistinctKeywords(String sql) {
		...
		patternString.append("\\s*").append(buildRegexStr("select")).append(
				"\\s*").append(buildRegexStr("distinct"));
